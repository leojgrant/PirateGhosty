...
def test_get_notification_history_success(notification_history_adapter_config: NotificationHistoryAdapterConfig,
    ...
    response = get_notification_history.get_notification_history(GetNotificationHistoryRequest(user_id=mock_notification.user_id))
    
    # Assert
    assert isinstance(response,GetNotificationHistoryResponse)
    assert all(isinstance(n, NotificationDto) for n in response.notifications)
    assert response.status == AdapterOperationStatus.SUCCESS
    assert response.message == "Successfully queried the notification history table."
